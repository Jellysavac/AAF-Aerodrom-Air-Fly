...
class Admin extends Component {
    ...
    state = {
        showFormForTicket: false, flights: [],
    ...
    handleFlightSubmit = event => {
        ...
        axios.post('http://localhost:8080/AirFly/let/addFlight', {
            type: this.state.type, seats: this.state.seats, date: this.state.date,
        ...
        return (
            <div>
                <Form onSubmit={this.handleFlightSubmit}>
                    <Form.Row style={{ paddingLeft: 10, paddingRight: 10 }}>
                        <Form.Group as={Col} controlId="formGridPolazniAerodrom">
                            <Form.Label>Polazni aerodrom</Form.Label>
                            <Form.Control as="select" onChange={(e) => this.setState({ departureId: e.target.value.substring(0, e.target.value.indexOf(",")) })} >
                                <option defalut>Ime i Grad</option>
                                {this.state.airports.map(response => <option>{response.id}, {response.naziv},{response.grad}</option>)}
                            </Form.Control>
                        </Form.Group>

                        <Form.Group as={Col} controlId="formGridDolazniAerodrom">
                            <Form.Label>Dolazni aerodrom</Form.Label>
                            <Form.Control as="select" onChange={(e) => this.setState({ arrivalId: e.target.value.substring(0, e.target.value.indexOf(",")) })} >
                                <option defalut>Ime i Grad</option>
                                {this.state.airports.map(response => <option>{response.id}, {response.naziv},{response.grad}</option>)}
                            </Form.Control>
                        </Form.Group>
                    ...
                    <Form.Group controlId="formGridDatum" style={{ paddingLeft: 10, paddingRight: 10 }}>
                        <Form.Label>Datum</Form.Label>
                        <Form.Control type="date" placeholder="13.04.2020." value={this.state.date} onChange={(e) => this.setState({ date: e.target.value })} />
                    </Form.Group>

                    <Form.Row style={{ paddingLeft: 10, paddingRight: 10 }}>
                        <Form.Group as={Col} controlId="formGridBrojMesta">
                        ...
                        <Form.Group as={Col} controlId="formGridVrsta">
                        ...
                        <Form.Group as={Col} controlId="formGridKompanija">
                            <Form.Label>Kompanija</Form.Label>
                            <Form.Control as="select" onChange={(e) => this.setState({ companyId: e.target.value.substring(0, e.target.value.indexOf(",")) })} >
